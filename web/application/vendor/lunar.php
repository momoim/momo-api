...
class Lunar {
     ...
     * @param Integer $day 日
     */
    public static function asterism($month, $day)
    {
        $opt = (int)$month*100 + (int)$day;
        //3月21日-4月19日
        if(321 <=$opt && $opt <= 419) {
            return array(1, '白羊座');
        }
        ...
        //10月24日-11月22日
        if(1024 <=$opt && $opt <= 1122) {
            return array(8, '天蝎座');
        }
        
        //11月23日-12月21日
        if(1123 <=$opt && $opt <= 1221) {
            return array(9, '射手座');
        }
        
        //12月22日-1月19
        if((1222 <=$opt && $opt <= 1231) || (101 <=$opt && $opt <= 119)) {
            return array(10, '摩羯座');
        }
...
$lunar = new Lunar ();
var_dump($lunar->convertLunarToSolar($year, $month, $day, $isleap));
*/
