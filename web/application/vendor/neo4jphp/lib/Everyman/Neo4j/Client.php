...
class Client
{
	 ...
	 * @param array $options array('timeout' => 3);
	 */
	public function __construct($transport=null, $port=7474, $options = array())
	{
		try {
			if ($transport === null) {
				$transport = new Transport\Curl();
			} elseif (is_string($transport)) {
				$transport = new Transport\Curl($transport, $port, $options);
			}
		} catch (Exception $e) {
			if ($transport === null) {
				$transport = new Transport\Stream();
			} elseif (is_string($transport)) {
				$transport = new Transport\Stream($transport, $port, $options);
			}
		}
