...
class Call_Records_Controller extends Controller {
    ...
    {
        parent::__construct();
        $this->model = Call_Records_Model::instance();
    }
    ...
    public function apply_batch_number() {
    	...
    	$total_call = $data['total_call']?trim($data['total_call']):0;
    	if (empty($device_id)) {
    		$this->send_response(400, NULL,'401201:设备id为空');
    	}
    	if (empty($total_call)) {
    		$this->send_response(400, NULL,'401202:总通话记录条数为空');
    	}
    	$number = $this->model->apply_batch_number($this->user_id,$device_id,$phone_model,$total_call,$this->appid,$this->source);
    	if($number)
    ...
    public function backup() {
    	...
    	$data_type = $data['data_type']?trim($data['data_type']):'common';
    	if (empty($device_id)) {
    		$this->send_response(400, NULL,'401201:设备id为空');
    	}
		...
		if(empty($data['data']) || !is_array($data['data']) || count($data['data'])==0) {
			$this->send_response(400, NULL,'401205:通话记录数据非法');
		}
		foreach ($data['data'] as $v) {
			if(empty($v['call_id']))
				$this->send_response(400, NULL,'401206:call_id为空');
			if(empty($v['date']))
				$this->send_response(400, NULL,'401207:date为空');
			if(trim($v['number'])==='')
				$this->send_response(400, NULL,'401208:number为空');
			if(empty($v['duration']) && $v['duration']!=0)
				$this->send_response(400, NULL,'401209:duration为空');
			if(empty($v['type']))
				$this->send_response(400, NULL,'401210:type为空');
		}
    	if (count($data['data']) > 200) {
			$this->send_response(400, NULL,'401212:超过最大通话记录上传数量');
		}
    	$batch_info = $this->model->get_batch_info($this->user_id,$device_id,$batch_number);
    	if(!$batch_info) {
    		$this->send_response(400, NULL,'401213:备份批号非法');
    	}
		$result = $this->model->add_batch($this->user_id,$data,$data_type);
		$result['backup_total_call'] = $batch_info['backup_total_call'];
    ...
    public function backup_done() {
    	...
    	$batch_total_call = $this->model->get_call_count($this->user_id,$device_id,$batch_number);
    	if($batch_info['backup_total_call'] != $batch_total_call) {
    		$this->send_response(400, NULL,'401214:通话记录数量校验不一致');
    	}
		if($this->model->backup_done($this->user_id,$device_id,$batch_number,$batch_info['phone_model'],$batch_info['backup_total_call'],$this->appid,$this->source))
			$this->send_response(200, array('backup_total_call'=>$batch_info['backup_total_call']));
    ...
    public function cancel_backup() {
    	...
    	$total_call = $this->model->cancel_backup($this->user_id,$device_id,$batch_number);
    	if($total_call)
			$this->send_response(200, array('batch_total_call'=>$total_call));
    ...
    public function restore() {
    	...
    	$result = $this->model->get_call_batch($this->user_id,$from_device_id,$from_batch_number,$start,$size);
    	if($result) {
