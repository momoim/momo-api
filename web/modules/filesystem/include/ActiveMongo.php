...
/*
  +---------------------------------------------------------------------------------+
  | Forked from César Rodas by huangfj                                              |
  +---------------------------------------------------------------------------------+
*/
...
abstract class ActiveMongo implements Iterator, Countable, ArrayAccess
{
     ...
     *  @return void
     */
    final public static function connect($db, $host='localhost', $user = NULL, $pwd=NULL, $opt=array())
    {
        self::$_host = $host;
        self::$_db   = $db;
        self::$_user = $user;
     ...
     */
    final public function _getConnection()
    {
        if (is_NULL(self::$_conn)) {
            if (is_NULL(self::$_host)) {
                self::$_host = 'localhost';
            }
            self::$_conn = Core::getMongo(self::$_host, self::$_opt);
        }
        if (isset($this)) {
     ...
     */
    final public function _getCollection()
    {
        if (isset($this)) {
    ...
    final function _find($_id = NULL, $fields = null, $use_document_vars = NULL, $findOne = false)
    {
        ...
        if ($use_document_vars || ($use_document_vars === NULL && $_id === NULL))
        {
          ...
          if($use_document_vars && is_array($use_document_vars)){
          	$vars=$use_document_vars;
          }
