...
class OAuthRequest { 
     ...
     */ 
    public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL, $timestamp='0') { 
        @$parameters or $parameters = array(); 
...
{ 
     ...
     * @return void 
     */ 
    function __construct( $akey , $skey , $accecss_token , $accecss_token_secret , $timestamp) 
    { 
        $this->oauth = new WeiboOAuth( $akey , $skey , $accecss_token , $accecss_token_secret, $timestamp); 
    } 
    ...
    { 
        return $this->oauth->get(API_PATH.'statuses/public_timeline.json');
    } 
    ...
    { 
        return $this->oauth->get(API_PATH.'statuses/home_timeline.json');
    } 
    ...
    { 
        ...
        return $this->oauth->post( API_PATH.'photo/upload.json' , $param , true, '', true ); 
    } 
    ...
    { 
        ...
        return $this->oauth->get(API_PATH.'statuses/comments.json' , $param );
...
class WeiboOAuth { 
     ...
     * @ignore 
     */
    public $host = API_PATH;
    //public $host = "http://api.91.com/";
     ...
     */ 
    function __construct($consumer_key, $consumer_secret, $oauth_token = NULL, $oauth_token_secret = NULL, $timestamp) { 
     ...
     * @return mixed 
     */ 
    function get($url, $parameters = array(), $is_header = FALSE) { 
        $response = $this->oAuthRequest($url, 'GET', $parameters, false, '', $is_header); 
        return $response; 
    } 
     ...
     * @param bool $multi 是否图片
     * @param json $data JSON数据
     * @return mixed 
     */ 
    function post($url, $parameters = array() , $multi = false, $data = '', $is_header= false) { 
        
        $response = $this->oAuthRequest($url, 'POST', $parameters , $multi , $data, $is_header); 
        return $response; 
    } 
     ...
     * @return string 
     */ 
    function oAuthRequest($url, $method, $parameters , $multi = false, $data = '', $is_header = false) { 

        if (strrpos($url, 'http://') !== 0 && strrpos($url, 'http://') !== 0) { 
            $url = "{$this->host}{$url}.{$this->format}"; 
        }
        // echo $url ;
        $request = OAuthRequest::from_consumer_and_token($this->consumer, $this->token, $method, $url, $parameters, $this->timestamp);
        $request->sign_request($this->sha1_method, $this->consumer, $this->token);
        switch ($method) { 
        case 'GET': 
             return $this->http($request->to_url(), 'GET', $data, $multi, '', $is_header); 
        case 'POST':
            return $this->http($request->get_normalized_http_url(), 'POST', $multi ? $request->to_postdata($multi) : $data, $multi, $request->to_header(), $is_header); 
        default: 
     ...
     */ 
    function http($url, $method, $postfields = NULL , $multi = false, $header = NULL, $is_header = false) { 
...
*/
        ...
        if ($header) {
            if( !$multi ) {
                $header_array2 = array("Content-Type: application/json ");
            }
             ...
             */
            array_push($header_array2,  'Expect:');
        }
